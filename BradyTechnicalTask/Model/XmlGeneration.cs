...
namespace BradyTechnicalTask.Model
{
    public class XmlGeneration
    {
        ...
        {
            var data = MapXmlToGenerationReport(inputFolderPath+ Reference.BackSlash + filename);
        ...
        private void GenerateOutPutFile(GenerationOutput generationOutput, string filename, string inputFolderPath)
        {
            ...
            var filePath = ConfigurationManager.AppSettings["OutputFolder"] + Reference.BackSlash + outpuytFilename.Replace(Reference.XmlExtension, string.Empty) + "-Result.xml";
